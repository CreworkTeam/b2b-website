              ...
              {roadmapSteps.map((step, index) => {
                ...
                return (
                  <li key={step.title} className="relative flex gap-4 pb-4.5 last:pb-0">
                    ...
                    <span
                      className={`z-10 mt-px inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${isActive ? 'bg-black text-white' : 'bg-[#E7E2D7] text-black'
                        } ${monoFontClass}`}
                    >
                      {index + 1}
                    </span>
